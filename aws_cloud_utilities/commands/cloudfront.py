...
@click.option(
    "--output-file", help="Output file for results (supports .json, .yaml, .csv)"
)
@click.pass_context
def update_logging(
    ctx: click.Context,
    ...
    try:
        ...
        if setup_alarms and remove_alarms:
            console.print(
                "[red]Error: --setup-alarms and --remove-alarms cannot be used together[/red]"
            )
            raise click.Abort()
        ...
        if not log_bucket and not setup_alarms and not remove_alarms:
            ...
            console.print(
                "[dim]Use --log-bucket to enable CloudFront logging configuration[/dim]"
            )
        ...
        if setup_alarms and sns_topic:
            ...
            if sns_topic_arn:
                ...
                console.print(
                    f"[yellow]⚠️  SNS topic '{sns_topic}' not found in region {target_region}[/yellow]"
                )
                console.print(
                    "[dim]Alarms will be created without notification actions[/dim]"
                )
...
def list_distributions(
    ...
    try:
        ...
        distributions_data = _get_all_distributions(
            cf_client, include_disabled, show_logging_status
        )
        ...
        if distributions_data:
            ...
            if output_file:
                ...
                console.print(
                    f"[green]Distributions list saved to:[/green] {output_path}"
                )
        else:
...
@click.option(
    "--show-config", is_flag=True, help="Show detailed distribution configuration"
)
@click.option(
    "--output-file", help="Output file for distribution details (supports .json, .yaml)"
)
@click.pass_context
def distribution_details(
    ...
    try:
        console.print(
            f"[blue]Getting details for CloudFront distribution: {distribution_id}[/blue]"
        )
        ...
        basic_info["Logging Enabled"] = (
            "Yes" if logging_config.get("Enabled", False) else "No"
        )
        ...
        if output_file:
            ...
            console.print(
                f"[green]Distribution details saved to:[/green] {output_path}"
            )
...
@click.option(
    "--output-file", help="Output file for invalidation details (supports .json, .yaml)"
)
@click.pass_context
def invalidate(
    ctx: click.Context, target: str, paths: Tuple[str, ...], output_file: Optional[str]
) -> None:
    ...
    try:
        ...
        console.print(
            f"[dim]Paths to invalidate: {', '.join(invalidation_paths)}[/dim]"
        )
        ...
        # Create invalidation
        invalidation_result = _create_invalidation(
            cf_client, distribution_id, invalidation_paths
        )
        ...
        result_data = {
            ...
            "Caller Reference": invalidation_result["Invalidation"][
                "InvalidationBatch"
            ]["CallerReference"],
            "Created At": invalidation_result["Invalidation"]["CreateTime"].strftime(
                "%Y-%m-%d %H:%M:%S UTC"
            ),
        }
        ...
        if output_file:
            ...
            save_to_file(save_data, output_path, file_format)
            console.print(
                f"[green]Invalidation details saved to:[/green] {output_path}"
            )
...
def _create_invalidation(
    cf_client, distribution_id: str, paths: List[str]
) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(
            f"Error creating invalidation for distribution {distribution_id}: {e}"
        )
        raise


def _get_sns_topic_arn(
    aws_auth: AWSAuth, topic_name: str, region: str
) -> Optional[str]:
...
def _update_cloudfront_distributions(
    ...
    with Progress(
        ...
        task = progress.add_task(
            "Processing distributions...", total=len(distributions)
        )

        results = []
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [
                executor.submit(process_distribution, dist) for dist in distributions
            ]
    ...
    summary = {
        ...
        "alarms_created": sum(
            len(r.get("alarms_result", {}).get("alarms_created", [])) for r in results
        ),
        "alarms_removed": sum(
            len(r.get("alarms_result", {}).get("alarms_removed", [])) for r in results
        ),
        "alarm_errors": sum(
            len(r.get("alarms_result", {}).get("errors", [])) for r in results
        ),
        "results": results,
    }
...
def _update_single_distribution(
    ...
    try:
        ...
        if log_bucket:
            needs_update = not result["logging_enabled"] or not result[
                "current_bucket"
            ].startswith(log_bucket)
...
def _enable_distribution_logging(
    aws_auth: AWSAuth, distribution_id: str, log_bucket: str, log_prefix: str
) -> None:
...
def _display_update_results(
    config: Config, update_results: Dict[str, Any], dry_run: bool
) -> None:
    ...
    summary_display = {
        ...
        "Updated" if not dry_run else "Would Update": (
            ...
            if not dry_run
            else update_results["would_update_count"]
        ),
    ...
    cf_managed_to_update = [
        ...
        if r["stack_name"]
        and (not r["logging_enabled"] or r.get("would_update", False))
    ]
            ...
            cf_data.append(
                {
                    ...
                    "Current Logging": (
                        "Enabled" if dist["logging_enabled"] else "Disabled"
                    ),
