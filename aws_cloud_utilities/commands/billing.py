...
class CURManager:
    ...
    def create_cur_report(self, 
                         ...
                         additional_schema_elements: List[str] = None) -> bool:
        """
...
@click.option(
    "--output-file",
    help="Output file for CUR report details (supports .json, .yaml)"
)
@click.pass_context
def cur_details(
    ctx: click.Context,
...
@click.option(
    "--report-name",
    required=True,
    help="Name for the new CUR report"
)
@click.option(
    "--bucket",
    required=True,
    help="S3 bucket name for CUR delivery"
)
@click.option(
    "--prefix",
    default="cur-reports",
...
@click.option(
    "--prefix",
    default="",
    help="S3 prefix for CUR files (optional)"
)
@click.pass_context
def cur_validate_bucket(
    ctx: click.Context,
    bucket_name: str,
    prefix: str
) -> None:
