...
**Features:**
- 🚀 **High-Performance Parallel Processing**: Analyzes buckets concurrently using ThreadPoolExecutor (5-10x faster than sequential processing)
- 🔒 **Encryption Categorization**: Clear separation between SSE-S3 (AES256) and SSE-KMS encrypted buckets
- 🔑 **KMS Key Details**: View which KMS keys are being used for each bucket
- ⚠️ **Unencrypted Bucket Detection**: Identify buckets without default encryption
- 📊 **HTML Report Generation**: Creates beautiful, responsive HTML reports with statistics and details
- 📄 **JSON Export**: Also saves data in JSON format for programmatic access
